...
[source, java]
----
    public static final String MESSAGE_USAGE = COMMAND_WORD + ": Edits the remark of the exercise identified "
...
Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the exercise's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a exercise.

=== Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.exercise`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.zerotoone.ui.ExerciseCard`].
...
.PersonCard.java
----
public PersonCard(Person exercise, int displayedIndex) {
    //...
